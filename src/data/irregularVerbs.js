...
function getIrregulars() {
    // Gather irregular words from
    ...
    const rows = document.querySelectorAll("tr:not(.irregular-label)");

    const mapped = Array.from(rows).map((item) =>
        Array.from(item.children).map((item) => item.innerHTML)
    );
    console.log(mapped);
}
